...
F == (Cardinality(Replicas) - 1) \div 3
FastQuorumSize == F * 3 + 1 
NormalQuorumSize == F * 2 + 1
FastQuorums == {R \in SUBSET(Replicas) : Cardinality(R) >= FastQuorumSize }
NormalQuorums == {R \in SUBSET(Replicas) : Cardinality(R)  >= NormalQuorumSize }   
...
MReply == 2 \* Fast Reply Message
MCert == 3 
MCertReply == 4
MRepiar == 5
MSlowReply == 6 \* Slow Reply Message
MLogIndex == 7  \* LogIndex
MLogEntry == 8  \* Log entry, different from index, it includes command field, which can be large in practice
