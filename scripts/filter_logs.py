...
def parse_client():
    ...
    try:
        for line in read_log_lines():
            ...
            if path == "fast":
                if last_commit != "fast":
                    ...
                    print(
                        f"Non-fast path period start={(last_fast_time - start_time).total_seconds()} end={(tags['time'] - start_time).total_seconds()} "
                    )
                ...
                if last_commit == "fast":
                    print(
                        f"Starting non-fast period at {(last_fast_time - start_time).total_seconds()}"
                    )
            ...
            if tags["time"] > start_time + interval * datetime.timedelta(minutes=1):
                print(
                    f"Minute ending {(start_time + interval * datetime.timedelta(minutes=1)).strftime('%H:%M')} summary:"
                )
                # TODO more stats

                total_commits = sum(len(commits[path]) for path in commits)
                total_latency = sum(
                    sum(c["latency"] for c in commits[path]) for path in commits
                )

                print(f"\tnum_commits={total_commits} ")
                print(
                    f"\tavg_latency={0 if total_latency == 0 else total_latency / total_commits}"
                )
                ...
                for path in commits:
                    ...
                    if len(commits[path]) > 0:
                        ...
                        print(
                            f"\t{path}_p95_latency = {np.percentile(latencies, 95):.0f} us"
                        )
                        print(
                            f"\t{path}_p99_latency = {np.percentile(latencies, 99):.0f} us"
                        )
                ...
                if runtime > 0:
                    ...
                    if last_commit != "fast":
                        cur_non_fast_seconds += (
                            tags["time"] - last_fast_time
                        ).total_seconds()

                    print(
                        f"Percent time in fast path (so far): {(runtime - cur_non_fast_seconds) / runtime:0.3f}"
                    )
                interval += 1
        ...
        print(
            f"Percent time in fast path: {(runtime - non_fast_seconds) / runtime:0.3f}"
        )
        for path in total_latencies:
            print(
                f"Number of {path} commits: {counts[path]}, average latency: {total_latencies[path] / max(1, counts[path]):.0f}"
            )
