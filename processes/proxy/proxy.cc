...
void Proxy::ForwardRequests()
{
    ...
    MessageHandlerFunc handleRequest = [this, &context](MessageHeader *hdr, void *body, Address *sender) {
        ...
        if (hdr->msgType == MessageType::MEASUREMENT_REPLY) {
            ...
            VLOG(2) << "Buffering (" << inReq.client_id() << ", " << inReq.client_seq() << ") deadline=" << outReq.deadline()
                    << " latencyBound=" << latencyBound_ << " now=" << GetMicrosecondTimestamp() << " batchSize=" << domReqBatchBuffer_.size();
            ...
            if (domReqBatchBuffer_.size() >= proxyBatchMaxCount_ || curBatchDelay_ >= proxyBatchMaxDelay_) {
                ...
                for (int i = 0; i < numReceivers_; i++) {
                    ...
                    VLOG(1) << "Forwarding batched req to " << receiverAddrs_[i].ip() << ":"
                            << receiverAddrs_[i].port_ << " msgType=" << (int) hdr->msgType << " msgLen=" << hdr->msgLen;
            ...
            isFirstReq = false;
            VLOG(2) << "Forwarding (" << inReq.client_id() << ", " << inReq.client_seq() << ") deadline=" << outReq.deadline()
                    << " latencyBound=" << latencyBound_ << " now=" << GetMicrosecondTimestamp();
            
            if (numForwarded_ % 10000 == 0) {
                VLOG(1) << "Forwarding request number " << numForwarded_ + 1 << " at time " << GetMicrosecondTimestamp();
            }
            numForwarded_++;
