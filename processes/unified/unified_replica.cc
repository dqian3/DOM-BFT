    ...
    , viewChangeNum_(viewChangeNum)
{
    ...
    for (int i = 0; i < numVerifyThreads_; i++) {
        verifyThreads_.emplace_back(&UnifiedReplica::verifyMessagesThd, this);
    }
    ...
    processThread_ = std::thread(&UnifiedReplica::processMessagesThd, this);

    LOG(INFO) << "UnifiedReplica initialized successfully";
}
...
void UnifiedReplica::run()
{
    endpoint_->Connect();
    LOG(INFO) << "Starting unified replica main loop";
    endpoint_->LoopRun();

    LOG(INFO) << "UnifiedReplica exited cleanly";
}

void UnifiedReplica::handleMessage(MessageHeader *msgHdr, byte *msgBuffer, Address *sender)
{
    if (msgHdr->msgLen < 0) {
        return;
    }
...
void UnifiedReplica::receiveRequest(MessageHeader *hdr, byte *body, Address *sender)
{
#if FABRIC_CRYPTO
...
void UnifiedReplica::forwardRequest(const DOMRequest &request)
{
...
void UnifiedReplica::checkDeadlines()
{
    std::lock_guard<std::mutex> guard(deadlineQueueMtx_);
...
void UnifiedReplica::receiverVerifyThd(int threadId)
{
...
void UnifiedReplica::verifyMessagesThd()
{
...
void UnifiedReplica::processMessagesThd()
{
    // TODO we do some redundant work deserializing messages here
    std::vector<byte> msg;
...
void UnifiedReplica::checkTimeouts()
{
    uint64_t now = GetMicrosecondTimestamp();
...
void UnifiedReplica::processClientRequest(const proto::ClientRequest &request, bool queued)
{
...
void UnifiedReplica::holdAndSwapCliReq(const proto::ClientRequest &request)
{
    uint32_t clientId = request.client_id();
    uint32_t clientSeq = request.client_seq();
    if (!heldRequest_) {
...
void UnifiedReplica::processCert(const Cert &cert)
{
...
void UnifiedReplica::processReply(const proto::Reply &reply, std::span<byte> sig)
{
...
void UnifiedReplica::processSnapshot(const AppSnapshot &snapshot, uint32_t round)
{
...
void UnifiedReplica::startCheckpoint(bool createSnapshot)
{
...
void UnifiedReplica::sendSnapshotRequest(uint32_t replicaId, uint32_t targetSeq)
{
...
void UnifiedReplica::processCommit(const dombft::proto::Commit &commit, std::span<byte> sig)
{
...
void UnifiedReplica::processSnapshotRequest(const SnapshotRequest &request)
{
...
void UnifiedReplica::processSnapshotReply(const dombft::proto::SnapshotReply &snapshotReply)
{
...
void UnifiedReplica::processRepairClientTimeout(const dombft::proto::RepairClientTimeout &msg, std::span<byte> sig)
{
    if (repair_) {
...
void UnifiedReplica::processRepairReplicaTimeout(const dombft::proto::RepairReplicaTimeout &msg, std::span<byte> sig)
{
...
void UnifiedReplica::processRepairReplyProof(const dombft::proto::RepairReplyProof &msg)
{
...
void UnifiedReplica::processRepairTimeoutProof(const dombft::proto::RepairTimeoutProof &msg)
{
    // Ignore repeated repair triggers
    if (repair_) {
...
void UnifiedReplica::processRepairStart(const RepairStart &msg, std::span<byte> sig)
{
...
void UnifiedReplica::processRepairDone(const RepairDone &msg)
{
...
LogSuffix &UnifiedReplica::getRepairLogSuffix()
{
...
bool UnifiedReplica::verifyCert(const Cert &cert)
{
...
bool UnifiedReplica::verifyRepairReplyProof(const RepairReplyProof &proof)
{
...
bool UnifiedReplica::verifyRepairTimeoutProof(const RepairTimeoutProof &proof)
{
...
bool UnifiedReplica::verifyCheckpoint(const LogCheckpoint &checkpoint)
{
...
bool UnifiedReplica::verifyRepairLog(const RepairStart &log)
{
...
bool UnifiedReplica::verifyRepairProposal(const RepairProposal &proposal)
{
...
bool UnifiedReplica::verifyViewChange(const PBFTViewChange &viewChange)
{
...
bool UnifiedReplica::verifyRepairDone(const RepairDone &done)
{
...
void UnifiedReplica::startRepair()
{
    repair_ = true;
...
void UnifiedReplica::finishRepair(const std::vector<::ClientRequest> &abortedReqs)
{
    repair_ = false;
    repairViewStart_ = 0;
...
void UnifiedReplica::tryFinishRepair()
{
    if (!repair_) {
        return;
    }
...
void UnifiedReplica::sendRepairSummaryToClients()
{
...
std::string UnifiedReplica::getProposalDigest(const dombft::proto::RepairProposal &proposal)
{
...
void UnifiedReplica::startViewChange()
{
...
void UnifiedReplica::doPrePreparePhase(uint32_t round)
{
    if (!isPrimary()) {
...
void UnifiedReplica::doPreparePhase()
{
...
void UnifiedReplica::doCommitPhase()
{
...
void UnifiedReplica::processPrePrepare(const PBFTPrePrepare &msg)
{
    if (msg.round() < round_) {
...
void UnifiedReplica::processPrepare(const PBFTPrepare &msg, std::span<byte> sig)
{
...
void UnifiedReplica::processPBFTCommit(const PBFTCommit &msg, std::span<byte> sig)
{
...
void UnifiedReplica::processPBFTViewChange(const PBFTViewChange &msg, std::span<byte> sig)
{
...
void UnifiedReplica::processPBFTNewView(const PBFTNewView &msg)
{
