...
#define Pthread_spin_init(l, pshared)       rrr_verify(pthread_spin_init(l, (pshared)) == 0)
#define Pthread_spin_lock(l)                rrr_verify(pthread_spin_lock(l) == 0)
#define Pthread_spin_unlock(l)              rrr_verify(pthread_spin_unlock(l) == 0)
#define Pthread_spin_destroy(l)             rrr_verify(pthread_spin_destroy(l) == 0)
#define Pthread_mutex_init(m, attr)         rrr_verify(pthread_mutex_init(m, attr) == 0)
#define Pthread_mutex_lock(m)               rrr_verify(pthread_mutex_lock(m) == 0)
#define Pthread_mutex_unlock(m)             rrr_verify(pthread_mutex_unlock(m) == 0)
#define Pthread_mutex_destroy(m)            rrr_verify(pthread_mutex_destroy(m) == 0)
#define Pthread_cond_init(c, attr)          rrr_verify(pthread_cond_init(c, attr) == 0)
#define Pthread_cond_destroy(c)             rrr_verify(pthread_cond_destroy(c) == 0)
#define Pthread_cond_signal(c)              rrr_verify(pthread_cond_signal(c) == 0)
#define Pthread_cond_broadcast(c)           rrr_verify(pthread_cond_broadcast(c) == 0)
#define Pthread_cond_wait(c, m)             rrr_verify(pthread_cond_wait(c, m) == 0)
#define Pthread_create(th, attr, func, arg) rrr_verify(pthread_create(th, attr, func, arg) == 0)
#define Pthread_join(th, attr)              rrr_verify(pthread_join(th, attr) == 0)
...
public:
    ...
    void wait(Mutex &m) { Pthread_cond_wait(&cv_, &m.m_); }
...
public:
    explicit ScopedLock(Lockable *lock)
        : m_(lock)
    {
        m_->lock();
    }
 ...
 * Thread safe queue.
 */
template <class T> class Queue : public NoCopy {
    std::list<T> *q_;
