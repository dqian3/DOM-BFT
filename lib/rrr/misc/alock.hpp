...
protected:
    ...
    uint64_t get_next_id() { return next_id_++; }

    virtual uint64_t vlock(
        const std::function<void(uint64_t)> &yes_callback, const std::function<void(void)> &no_callback, type_t type,
...
public:
    ...
    virtual uint64_t lock(
        const std::function<void(uint64_t)> &yes_callback, const std::function<void(void)> &no_callback,
        type_t type = WLOCK,
        int64_t priority = 0,   // lower value has higher priority
        const std::function<int(void)> &wound_callback = std::function<int(void)>()
    )
    {
        return vlock(yes_callback, no_callback, type, priority, wound_callback);
    }

    virtual uint64_t lock(
        const std::function<void(void)> &yes_callback, const std::function<void(void)> &no_callback,
        type_t type = WLOCK,
        int64_t priority = 0,   // lower value has higher priority
        const std::function<int(void)> &wound_callback = std::function<int(void)>()
    )
    {
        std::function<void(uint64_t)> _yes_callback = [yes_callback](uint64_t id) { yes_callback(); };
        return vlock(_yes_callback, no_callback, type, priority, wound_callback);
    }
...
protected:
    struct lock_req_t {
        typedef enum { WAIT, LOCK } lock_req_status_t;
        uint64_t id;
        int64_t priority;
        type_t type;
        ...
        lock_req_t(
            uint64_t _id, int64_t _priority, type_t _type, const std::function<void(uint64_t)> &_yes_callback,
            const std::function<void(void)> &_no_callback, lock_req_status_t _status = WAIT
        )
            : id(_id)
            , priority(_priority)
            , type(_type)
    ...
    typedef enum { WD_WAIT, WD_DIE } wd_status_t;
    ...
    {
        rrr_verify(lock_req.type == WLOCK && lock_req.status == lock_req_t::WAIT);
        status_ = WLOCKED;
    ...
    {
        rrr_verify(lock_req.type == RLOCK && lock_req.status == lock_req_t::WAIT);
        if (n_rlock_ == 0)
            status_ = RLOCKED;
        n_rlock_++;
        lock_req.status = lock_req_t::LOCK;
    ...
    void read_acquire(const std::vector<lock_req_t *> &lock_reqs)
    {
        ...
        for (it = lock_reqs.begin(); it != lock_reqs.end(); it++) {
            rrr_verify((*it)->type == RLOCK && (*it)->status == lock_req_t::WAIT);
            (*it)->status = lock_req_t::LOCK;
            to_calls.push_back(std::pair<std::function<void(uint64_t)>, uint64_t>((*it)->yes_callback, (*it)->id));
        }
    ...
    virtual uint64_t vlock(
        const std::function<void(uint64_t)> &yes_callback, const std::function<void(void)> &no_callback, type_t type,
        int64_t priority, const std::function<int(void)> &
    );
    ...
    void sanity_check()
    {
        ...
        for (; it != requests_.end(); it++) {
            if (!acquired_check) {
                if (status_ == WLOCKED) {
                    acquired_check = true;
                    rrr_verify(it == requests_.begin());
                    rrr_verify(it->type == WLOCK);
                    rrr_verify(it->status == lock_req_t::LOCK);
                } else {
                    rrr_verify(status_ == RLOCKED);
                    if (it->type == RLOCK) {
                        rrr_verify(it->status == lock_req_t::LOCK);
                        n_r_locked++;
                    } else {
                        rrr_verify(it->status == lock_req_t::WAIT);
                        acquired_check = true;
                        rrr_verify(n_r_locked == n_rlock_);
                    }
                }
            } else {
                rrr_verify(it->status == lock_req_t::WAIT);
        ...
        rrr_verify(num_w == n_w_in_queue_);
        rrr_verify(num_r == n_r_in_queue_);
    }
...
protected:
    struct lock_req_t {
        ...
        lock_req_t(
            uint64_t _id, int64_t _priority, type_t _type, const std::function<void(uint64_t)> &_yes_callback,
            const std::function<void(void)> &_no_callback, const std::function<int(void)> &_wound_callback,
    ...
    virtual uint64_t vlock(
        const std::function<void(uint64_t)> &yes_callback, const std::function<void(void)> &no_callback, type_t type,
        int64_t priority, const std::function<int(void)> &wound_callback
    );
...
protected:
    virtual uint64_t vlock(
        const std::function<void(uint64_t)> &yes_callback, const std::function<void(void)> &no_callback, type_t type,
        int64_t, const std::function<int(void)> &
    );
...
public:
    ...
    public:
        ...
        ALockReq(
            uint64_t id, type_t type, const std::function<void(uint64_t)> &yes_cb, const std::function<void()> &no_cb,
            uint64_t tm_out
        )
            : id_(id)
            , type_(type)
...
public:
    ...
    ALockGroup(int64_t priority = 0, const std::function<int(void)> &wound_callback = std::function<int(void)>())
        : priority_(priority)
    ...
    void lock_all(const std::function<void(void)> &yes_cb, const std::function<void(void)> &no_cb);

    void unlock_all()
    {

        rrr_verify(cas_status(LOCK, UNLOCK));
        // abort all the locks.
