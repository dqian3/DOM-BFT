...
public:
    ...
    void submit(const std::string &buf, const std::function<void(void)> &cb = std::function<void(void)>());

    void submit(Marshal &m, const std::function<void(void)> &cb = std::function<void(void)>());

    void flush_buf();

    void callback(std::vector<std::pair<std::string, std::function<void(void)>>> *reqs);
    ...
    void set_batch(uint64_t time) { batch_time_ = time; }
