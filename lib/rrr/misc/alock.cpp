...
)
{
    ...
    if (status_ == FREE || (status_ == RLOCKED && type == RLOCK && n_w_in_queue_ == 0)) {
        // acquire lock
...
WaitDieALock::wd_status_t WaitDieALock::wait_die(type_t type, int64_t priority)
{
    ...
    case RLOCK: {
        ...
        rrr_verify(0);   // should be able to acquire the lock, no need to wait or die
    }
    default:
        rrr_verify(0);
    }
...
)
{
    ...
    requests_.emplace_back(id, priority, type, yes_callback, no_callback, wound_callback);
    ...
    case RLOCKED: {
        rrr_verify(front.type == RLOCK && front.status == lock_req_t::LOCK);
        bool new_acquired = false;
        std::vector<lock_req_t *> lock_reqs;
        std::list<lock_req_t>::iterator it = requests_.begin();
        for (; it != requests_.end(); it++) {
            if (it->status == lock_req_t::LOCK) {
                rrr_verify(it->type == RLOCK && new_acquired == false);
    ...
    case WLOCKED:
        rrr_verify(front.type == WLOCK && front.status == lock_req_t::LOCK);
        break;
    default:
        rrr_verify(0);
    }
...
)
{
    ...
    requests_.emplace_back(id, type, yes_callback, no_callback, 0);
    //    requests_.emplace_back(id, type);
    ALockReq &req = requests_.back();

    bool lockable = (status_ == FREE) || (status_ == RLOCKED && type == RLOCK);
    ...
    if (lockable) {
        ...
        req.alarm_id_ = alarm.add(tm_out, [this, &req]() { this->do_timeout(req); });
    } else {
        // tm_wait_ = 0
...
{
    rrr_verify(status_ == FREE && n_rlock_ == 0);
    ...
    for (; status_ == RLOCKED && it != requests_.end(); it++) {
        auto &next_req = *it;
        if (next_req.type_ == RLOCK && next_req.cas_status(ALockReq::WAIT, ALockReq::LOCK)) {
...
void ALockGroup::lock_all(const std::function<void(void)> &yes_cb, const std::function<void(void)> &no_cb)
{

    rrr_verify(cas_status(INIT, WAIT) || cas_status(LOCK, WAIT));
    ...
    db_ = new DragonBall(tolock_.size(), [this]() {
        if (this->cas_status(WAIT, LOCK)) {
            ...
            rrr_verify(this->get_status() == TIMEOUT);
            this->no_callback_();
