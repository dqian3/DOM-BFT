...
public:
    explicit CheckpointCollectors(uint32_t replicaId, uint32_t f): replicaId_(replicaId), f_(f){}

    inline CheckpointCollector& at(uint32_t seq){
        return collectors_.at(seq);
    }
